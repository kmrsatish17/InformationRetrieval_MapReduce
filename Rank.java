...
public class Rank extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		job4.setMapperClass(MapFour.class);
		job4.setReducerClass(ReduceFour.class);
		job4.setSortComparatorClass(MappComparator.class);
	...
	// Mapper class for fourth mapper
	public static class MapFour extends Mapper<LongWritable ,  Text ,  DoubleWritable , Text > {
		MapWritable mapWritable = new MapWritable();
		private Text fileWord = new Text();
		...
		public void map( LongWritable offset,  Text lineText,  Context context)
			...
			// Tokenizing the input to get the file name and TFIDF value
			StringTokenizer st = new StringTokenizer(line);
			DoubleWritable tfidfValue = null;
			
			while (st.hasMoreTokens()) {
			fileWord.set(st.nextToken());
			tfidfValue = new DoubleWritable(Double.parseDouble(st.nextToken()));
			// Writing the map output as Key = tfidfValue and value = fileWord
			context.write(tfidfValue,fileWord);
			}
			
		}
	}

	// Comparator class to compare the output of fourth Mapper
	public static class MappComparator extends WritableComparator {

		protected MappComparator() {
	...
	// Reducer class for fourth reducer
	public static class ReduceFour extends Reducer<DoubleWritable ,  Text ,  Text ,  DoubleWritable > {
		@Override 
		public void reduce( DoubleWritable filecount,  Iterable<Text > counts,  Context context)
				throws IOException,  InterruptedException {
			
			// Wrting the required output of fourth reducer
			for (Text fileword  : counts) {
				context.write(fileword,  filecount);
			}
		}
	}
